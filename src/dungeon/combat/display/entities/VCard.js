...
export class VCard extends Container {
    constructor(data) {
        ...
        this.state = createAnimatedButtonBehavior(this, {
    ...
    addCostIndicator() {
        ...
        onEnterFrame.watch(() => this.data.cost, v => {
    ...
    addValueIndicator() {
        ...
        if (this.data.type === "func") {
            if (this.data.mods) {
                const emojis = Object.keys(this.data.mods).map((k) => StatusEffectBlueprints[k]?.emoji || "N/A");
...
(function (VCard) {
    VCard.DESIGN_WIDTH = 500;
    VCard.DESIGN_HEIGHT = 700;
})(VCard || (VCard = {}));
