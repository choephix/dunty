...
import { CombatantStatus } from "@dungeon/combat/state/CombatState";
...
export class VCombatant extends Container {
    constructor(data) {
        ...
        this.breathingOptions = {};
        this.onEnterFrame = createEnchantedFrameLoop(this);
    ...
    //// Builder Methods
    startBreathing({ start = Math.random(), speed = 4 + Math.random(), skew = 0.0 } = {}) {
    ...
    intializeAnimationReactors() {
        const { status } = this.data;
        this.onEnterFrame.watch.array(() => [status.health, status.block, status.retaliation || 0], async ([health, block, retaliation], [prevHealth, prevBlock, prevRetaliation]) => {
            const Ani = VCombatantAnimations;
            if (health < prevHealth)
                await (health <= 0 ? Ani.die(this) : Ani.hurt(this));
            if (health > prevHealth)
                await (health > 0 ? Ani.buffHealth(this) : Ani.enter(this));
            if (block > prevBlock)
                await Ani.buffBlock(this);
            if (retaliation > prevRetaliation)
                await Ani.buffRetaliation(this);
        }, true);
        const noFloatyTextKeys = ["health"];
        this.onEnterFrame.watch.properties(() => status, (current, prev) => {
    ...
    addStatusIndicators() {
        ...
        this.onEnterFrame.watch.properties(() => status, () => statusIndicator.update(status), true);
        return statusIndicator;
    }
    addIntentionIndicator() {
        const { drawPile } = this.data.cards;
        const intentionIndicator = new IntentionIndicators();
        this.addChild(intentionIndicator);
        this.onEnterFrame.watch(() => this.thought || drawPile[0], v => typeof v === "string"
            ? intentionIndicator.updateFromText(this.data, v)
            : intentionIndicator.updateFromUpcomingCards(this.data, Combat.current), true);
    ...
    addEnergyIndicator() {
        ...
        this.addChild(intentionIndicator);
        this.onEnterFrame.watch(() => this.data.energy, v => {
...
class StatusEffectIndicators extends Container {
    ...
    update(status) {
        ...
        const froms = spritesArray.map(child => [child, child.position.clone()]);
        ...
        froms.forEach(([child, { x, y }]) => !child.isNew && new TemporaryTweeener(child).from(child, { x, y, duration: 0.15 }));
        this.animateInNewChildren();
    }
    createStatusIndicator(key, value) {
        const { displayPriority, impactAlignment } = StatusEffectBlueprints[key];
        function getColors(key) {
            if (key === "health")
                return [0x70c050, 0x107010, 0x105010];
            if (key === "block")
                return [0x4050f0, 0x101090];
            if (impactAlignment === StatusEffectImpactAlignment.NEUTRAL)
                return [0x4050f0, 0x101090];
            if (impactAlignment === StatusEffectImpactAlignment.POSITIVE)
                return [0x40c0f0, 0x106090];
            if (impactAlignment === StatusEffectImpactAlignment.NEGATIVE)
                return [0xf04040, 0x901010];
        }
        const colors = {
...
function getStatusEffectEmojifiedString(key, value) {
    const emoji = getStatusEffectEmojiOnly(key);
...
class IntentionIndicators extends Container {
    ...
    afterUpdate() {
        const froms = [...this.sprites.values()].map(child => [child, child.position.clone()]);
    ...
    updateFromUpcomingCards(actor, game) {
        this.clear();
    ...
    createIndicatorFromText(str, colors) {
    ...
    createIndicatorFromCard(actor, card) {
        const emojifySingleCardIntention = (card) => {
            const { type } = card;
            if (type === "atk") {
            ...
            if (type === "func") {
                try {
                    if (!card.isBloat)
                        throw new Error("Not a bloat");
                    const mod = [...Object.keys(card.mods)][0];
    ...
    animateInNewChildren() {
        for (const [card, sprite] of this.sprites) {
            if (sprite.isNew) {
                ...
                const fx = spawnSpriteWave("https://undroop-assets.web.app/davinci/3/radial-4.png", { pixi: { scale: 2 }, duration: 2 }, { scale: 0, tint: waveColor, blendMode: BLEND_MODES.ADD });
                return sprite.addChild(fx);
            }
        }
    }
}
//# sourceMappingURL=VCombatant.js.map