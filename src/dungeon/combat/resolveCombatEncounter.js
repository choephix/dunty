...
import { CardTarget, CombatantStatus } from "@dungeon/combat/state/CombatState";
...
export async function resolveCombatEncounter() {
    ...
    const combatantsDictionary = new Map();
    function composeSide(state, leftSide) {
    ...
    const activeCombatant = new CurrentSelectionHelper({
        onSelect: combatant => {
    ...
    vhand.onCardClick = async (card) => {
        ...
        if (actor.cards.hand.length === 0) {
            ...
            await delay(0.8);
            if (activeCombatant.current === actor)
                endPlayerTurn();
        }
    };
    async function playCardFromHand(card, actor, targets) {
        try {
    ...
    async function getPlayerCardTargets(card, actor) {
        switch (card.target) {
    ...
    async function resolveCardEffect(card, actor, targets) {
                ...
                for (const [key, delta] of CombatantStatus.entries(mods)) {
                    for (const target of targets) {
                        ...
                        if (key === "stunned" || key === "frozen") {
                            for (const _ of range(delta))
                                target.cards.addCardTo(generateBloatCard(key), target.cards.drawPile);
                        }
                        await delay(0.1);
                    }
                    await delay(0.55);
    ...
    async function playerSelectTarget(candidates) {
        ...
        const chosen = await new Promise(resolve => {
            for (const candidate of candidates) {
                ...
                createAnimatedButtonBehavior(rect, {
    ...
    async function performTargetedAttack(target, attacker, card) {
        const atkPwr = combat.faq.calculateAttackPower(card, attacker);
        const { directDamage, blockedDamage, returnedDamage: reflectedDamage, healingDamage, } = combat.faq.calculateDamage(atkPwr, target, attacker);
    ...
    async function performSpreadAttack(targets, attacker, card) {
        const atkPwr = combat.faq.calculateAttackPower(card, attacker);
        const damageResults = targets.map(target => [target, combat.faq.calculateDamage(atkPwr, target, attacker)]);
        const vatk = combatantsDictionary.get(attacker);
        await VCombatantAnimations.attack(vatk);
    ...
    const endTurnButtonBehavior = createAnimatedButtonBehavior(endTurnButton, {
        onClick: () => {
            endPlayerTurn();
        },
    ...
    function addSpotlightEffect() {
        ...
        spotlights.onEnterFrame.watch(() => activeCombatant.current, combatant => {
...
//# sourceMappingURL=resolveCombatEncounter.js.map