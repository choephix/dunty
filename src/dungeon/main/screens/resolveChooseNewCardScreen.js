...
export async function resolveChooseNewCardScreen(cardsCount) {
    ...
    const chosenVCard = await new Promise(resolve => {
    ...
    await vscene.tweeener.to(vcards.filter(vc => vc !== chosenVCard), { pixi: { scale: 0 }, duration: 0.25, ease: "back.in", stagger: 0.1 });
    vcards.forEach(vcard => vcard.destroy());
    console.log(`🤍 You chose `, chosenVCard.data);
    await delay(0.4);
    vscene.playHideAnimation().then(() => vscene.destroy());
}
//# sourceMappingURL=resolveChooseNewCardScreen.js.map