...
import { randomSigned } from "@sdk/utils/random";
import { Viewport } from "pixi-viewport";
import { gsap } from "gsap";
import { range } from "@sdk/utils/range";
import { BossNode, RegularNode, Node } from "./Node";
import { Graphics } from "@pixi/graphics";
export function initializeDungeonFloor(app) {
    ...
    for (const node of nodesParent.children) {
        for (const otherNode of nodesParent.children) {
            if (node instanceof Node && otherNode instanceof Node) {
                if (node == otherNode)
                    continue;
                if (links.get(otherNode)?.includes(node))
                    continue;
                if (!links.has(node))
                    links.set(node, []);
                ...
                if (distance > 380 + 8 * node.circleIndex)
                    continue;
...
//# sourceMappingURL=testFloor.js.map