...
initDebugging();
export function boot(applicationOptions = {}) {
    ...
    const onlyIfPageVisible = (callback) => () => void (document.visibilityState === "visible" && callback());
    const ticker = new Ticker();
    ticker.start();
    ticker.add(onlyIfPageVisible(() => app.render()), null, -100);
    ticker.add(onlyIfPageVisible(() => callOnEnterFrameRecursively(app.stage)), null, 99);
    app.ticker = ticker;
    return app;
}
