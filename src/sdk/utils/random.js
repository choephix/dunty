 ...
 */
export function chance(fraction) {
 ...
 * @param max Excluding
 */
export const randomInt = (max) => ~~(Math.random() * max);
export const randomIntBetweenIncluding = (min, max, lean = 1.0) => min + ~~(Math.random() ** lean * (max - min + 1));
// /**
//  * @param max the maximum positive value you can get. Minumum will be (-max). Defaults to one.
//  */
export const randomSigned = (max = 1) => Math.random() * max * 2.0 - max;
// /**
